...
inline DPoint
PointOfLine_Dis(DPoint P1, DPoint P2, double D)
{
...
inline DPoint
PointOfLine_XorY(DPoint P1, DPoint P2, double D, bool XorY=true)
{
    double k = (P2.y_-P1.y_)/(P2.x_-P1.x_);
    if(XorY)
        return DPoint(D,P1.y_+(D-P1.x_)*k);
    else
        return DPoint(P1.x_+(D-P1.y_)/k,D);
}
...
inline bool
VectorEqu(DPoint P1, DPoint P2,double e)
{
    double dis=(P1.x_-P2.x_)*(P1.x_-P2.x_)+(P1.y_-P2.y_)*(P1.y_-P2.y_);
    if(dis>e*e)
        return 0;
...
inline void
ExchangeSort(float* pData,char Count,char order[])
{
    int iTemp,otemp;
...
inline double
AreaofPolygon(vector<DPoint> _points)
{
    double _area_sum=0;
    for(int i=0; i<_points.size()-2;i++)
        _area_sum=_area_sum+fabs(Product(_points[0],_points[i+1],_points[i+2])/2);

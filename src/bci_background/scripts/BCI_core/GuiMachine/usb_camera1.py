...
class UsbCamera(object):
    ...
    def capture(self):
        self.sur = pygame.image.fromstring(self.cam.getImage().tostring(),(640,480),'RGB').convert()
    ...
    def reset(self,**argw):
        ...
        if self.update:
            if time.clock()-self.clk>self.pt:
                self.capture()
    ...
    def show(self):
        if self.visible and self.sur!=None: self.root.blit(self.sur,self.blitp)
...
global BP_4_35_N96;
% BPfilterSSVEP = fir1(96,[4 35].*2./200);
global sigpssvep_filter
sigpssvep_filter=[];

global ssvep_begin ssvep_end
global pre_phaseinsequence
phaseinsequence = bci_States.PhaseInSequence;
label = bci_States.label;
global  currenttrial subject_name subject_session subject_run subject_save
...
epoch=2;
out_result=0;
global  currt_k raw_outsg  result_win
...
if phaseinsequence >= 1
    ...
    sigpssvep_filter = filter(BP_4_35_N96, 1, signal(:,channel_ssvep));
    out_signal = [0,0,0];
    ...
    %
    %             % for qq = 1:num_SSVEP
    %             % YY = [sin(2*pi*f_ssvep(qq)*tt),cos(2*pi*f_ssvep(qq)*tt)];
    %             % [A,B,r] = canoncorr(sigssvep_slice,YY);
    %             % score_CCA(qq) = max(r);
    %             % end
    %
    ...
    %             out_signal = [index_CCA1,min(find(raw_outsg_ilen==max(raw_outsg_ilen))),out_result];%length(raw_outsg)];%max(raw_outsg_ilen)];,
    %             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %         end
    %
    %     end

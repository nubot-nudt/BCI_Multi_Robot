...
for loop=1:loop_times
    ...
    if right_decision==1
        human_decision=randsrc(1,1,[[1 2 3]; [0.9 0.05 0.05]]);
    elseif right_decision==2
        human_decision=randsrc(1,1,[[1 2 3]; [0.05 0.9 0.05]]);
    else
        human_decision=randsrc(1,1,[[1 2 3]; [0.05 0.05 0.9]]);
    end
    ...
    if human_decision~=robot_decision
        ...
        elseif human_decision==2
            tmp_label=randi([1,length(human_label_2)],1);
            BCI_signal=human_label_2(tmp_label,1:3);
        else
            tmp_label=randi([1,length(human_label_3)],1);
            BCI_signal=human_label_3(tmp_label,1:3);
        end
        human_decision=max(BCI_signal);
        if attack_or_defend==1
            probability_human=flipud(BCI_signal)/sum(BCI_signal);
        else
            probability_human=BCI_signal/sum(BCI_signal);
        end
